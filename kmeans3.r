...
clust= c()    #Have initialized these for now, don't know where I will use it lol!
x_cluster1=c()
y_cluster1=c()
x_cluster2=c()
y_cluster2=c()
...
#par(mfrow=c(2,2))
#plot(sample,xlab="x axis", ylab="y axis", pch=16, col="blue")   #Scatter plot for the data points
#points(centroid_1[1], centroid_1[2], pch=8, col="red")
#points(centroid_2[1],centroid_2[2], pch=8, col="red")
...
for(i in 1:10)
{
    ...
    #Making the comprehensive table to list the Euclidean and the cluster which the datapoint belongs to
    ...
    print(tab_cluster)
    #df=as.data.frame(t(tab_cluster))
    long=melt(tab_cluster)
    head(long)
    colnames(long) = c("X","Y")
    head(long)
    n=ggplot(long, aes(x=X, y=Y)) + geom_point()
    print(n)
